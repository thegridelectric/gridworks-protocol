...
class SingleReading(BaseModel):
    ...
    def as_type(self) -> bytes:
        """
        Serialize to the single.reading.000 representation designed to send in a message.
...
class SingleReadingMaker:
    ...
    @classmethod
    def type_to_tuple(cls, b: bytes) -> SingleReading:
        """
        ...
        Raises:
           GwTypeError: if the bytes are not a single.reading.000 type

        Returns:
            SingleReading instance
        """
        try:
            d = json.loads(b)
    ...
    def dict_to_tuple(cls, d: dict[str, Any]) -> SingleReading:
        """
        Translates a dict representation of a single.reading.000 message object
        ...
        if "Value" not in d2.keys():
            raise GwTypeError(f"dict missing Value: <{d2}>")
