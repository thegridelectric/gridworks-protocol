...
def test_thermistor_data_processing_config_generated() -> None:
    t = ThermistorDataProcessingConfig(
        channel_name="hp-ewt",
        terminal_block_idx=4,
        thermistor_make_model=MakeModel.TEWA__TT0P10KC3T1051500,
        data_processing_method=ThermistorDataMethod.SimpleBeta,
        data_processing_description="using a beta of 3977.",
    )
    ...
    d2["ThermistorMakeModel"] = MakeModel.TEWA__TT0P10KC3T1051500.value
    del d2["DataProcessingMethodGtEnumSymbol"]
    d2["DataProcessingMethod"] = ThermistorDataMethod.SimpleBeta.value
    ...
    assert Maker.dict_to_tuple(d2).thermistor_make_model == MakeModel.default()

    d2 = dict(d, DataProcessingMethodGtEnumSymbol="unknown_symbol")
    assert (
        Maker.dict_to_tuple(d2).data_processing_method == ThermistorDataMethod.default()
    )
