...
class SnapshotSpaceheat(BaseModel):
    ...
    @field_validator("snapshot_time_unix_ms")
    @classmethod
    ...
    def plain_enum_dict(self) -> Dict[str, Any]:
        ...
        latest_reading_list = []
        for elt in self.latest_reading_list:
    ...
    def as_type(self) -> bytes:
        """
        Serialize to the snapshot.spaceheat.001 representation designed to send in a message.
...
class SnapshotSpaceheatMaker:
    ...
    def type_to_tuple(cls, b: bytes) -> SnapshotSpaceheat:
        ...
        Raises:
           GwTypeError: if the bytes are not a snapshot.spaceheat.001 type
    ...
    def dict_to_tuple(cls, d: dict[str, Any]) -> SnapshotSpaceheat:
        """
        Translates a dict representation of a snapshot.spaceheat.001 message object
        ...
        if "SnapshotTimeUnixMs" not in d2.keys():
            raise GwTypeError(f"dict missing SnapshotTimeUnixMs: <{d2}>")
        if "LatestReadingList" not in d2.keys():
            raise GwTypeError(f"dict missing LatestReadingList: <{d2}>")
        ...
        for elt in d2["LatestReadingList"]:
            ...
            t = SingleReadingMaker.dict_to_tuple(elt)
